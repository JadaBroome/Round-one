...
for (i = 0; i < graduates.length; i++) {
  let colleges = graduates[i].university;
  {
    universities.push(colleges);
    universities.sort();
  }
}
...
for (let i = 0; i < graduates.length; i++) {
  // const contactName = graduates[i].first_name;
  // const contactEmail = graduates[i].email;
...
zooAnimals.forEach(item => {
  animalNames.push(
    `Name ${item.animal_name} Scientifiic ${item.scientific_name}.`
  );
});
...
const lowerCase = [];
zooAnimals.map(name => {
  lowerCase.push(name.animal_name.toLowerCase());
});
...
zooAnimals.filter(item => {
  largerPopulation.push(item.population < 5);
});
...
zooAnimals.reduce(function(total, item) {
